...
const sourceLabels: Record<SourceId, { label: string; sublabel: string; icon: typeof Wind }> = {
  ...
  field: { label: "Field observation", sublabel: "Human-provided text or photo", icon: MessageSquareText },
};
...
export default function Home() {
  ...
  const receiptMutation = trpc.aegis.recordReview.useMutation({
    onSuccess: async data => {
      setNotice(data.persistence === "unavailable" ? "Your public response was not stored because this deployment has no persistence service. Aegis did not act autonomously." : `Public human response #${data.receiptId} recorded. Aegis did not act autonomously.`);
      setReviewOpen(false); setReviewNote("");
      await Promise.all([assessmentQuery.refetch(), publicAssessmentQuery.refetch()]);
    },
  });

  const assessment = assessmentQuery.data?.assessment ?? publicAssessmentQuery.data?.assessment;
