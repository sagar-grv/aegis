...
| Field observation | Authenticated Aegis operator | Condition, observed wind, free-text note, optional bounded site photo, and attributed visual observation | Human ground truth and sensor contradiction check. |
...
Live public data is external evidence, not an instruction to take action. Aegis applies a deterministic confidence policy, shows missing or deliberately fault-injected source rows, and creates a refusal state when decisive weather evidence and field observation are unavailable. The operator can add only genuine observations; they are stored independently of the recommendation. An optional authenticated photo is constrained to neutral, schema-validated scene context and cannot make or execute an operational decision. This creates an accountable human-in-the-loop record rather than an autonomous control system.
